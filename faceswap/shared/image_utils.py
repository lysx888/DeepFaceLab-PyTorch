...
def imread_bgr_safe(path: Union[str, Path], flags: int = cv2.IMREAD_COLOR) -> Optional[npt.NDArray]:
    p = Path(path)
    if not p.exists():
        return None
    try:
        ...
        _logger.warning(f"imread_bgr_safe failed for {p}: {e}")
        return None


def imwrite_bgr_safe(path: Union[str, Path], img: npt.NDArray, params=None) -> bool:
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    try:
        ext = p.suffix or ".png"
        ok, data = cv2.imencode(ext, img, params) if params is not None else cv2.imencode(ext, img)
        if not ok:
            return False
        data.tofile(str(p))
        return True
    except Exception as e:
        _logger.warning(f"imwrite_bgr_safe failed for {p}: {e}")
        return False
