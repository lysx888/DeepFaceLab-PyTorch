...
_manual_device: Optional[str] = None  # None=自动探测; 'cpu'/'cuda'=固定选择(GUI右下角切换)


def set_manual_device(device: Optional[str]) -> None:
    """设置手动设备选择: 'cpu' / 'cuda' / None(恢复自动探测)。全局生效。"""
    global _manual_device
    _manual_device = device


def get_manual_device() -> Optional[str]:
    return _manual_device


def auto_select_device() -> "torch.device":
    import torch
    if _manual_device is not None:
        return torch.device(_manual_device)
