...
from .mb_v3_networks import recon_mobilenetv3_large
    ...
    def __init__(self, net_recon, use_last_fc=False, init_path=None):
        ...
        if not use_last_fc:
            self.final_layers = nn.ModuleList([
                ...
                conv1x1(last_dim, 80, bias=True), # alb layer
                conv1x1(last_dim, 3, bias=True),  # angle layer
                conv1x1(last_dim, 27, bias=True), # sh layer
...
func_dict = {
    ...
    'recon_mobilenetv3_large': (recon_mobilenetv3_large, 960),
}



def define_net_recon_mobilenetv3(net_recon, use_last_fc=False, init_path=None):
    return ReconNetWrapper_mobilenetv3(net_recon, use_last_fc=use_last_fc, init_path=init_path)

class ReconNetWrapper_mobilenetv3(nn.Module):
    fc_dim=257
    def __init__(self, net_recon, use_last_fc=False, init_path=None):
        super(ReconNetWrapper_mobilenetv3, self).__init__()
        self.use_last_fc = use_last_fc
        if net_recon not in func_dict:
            return  NotImplementedError('network [%s] is not implemented', net_recon)
        func, last_dim = func_dict[net_recon]
