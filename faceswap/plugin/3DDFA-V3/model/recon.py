...
class face_model:
    def __init__(self, args):
        ...
        if args.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load("assets/net_recon.pth", map_location=torch.device('cpu'), weights_only=False)['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load("assets/net_recon_mbnet.pth", map_location=torch.device('cpu'), weights_only=False)['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
