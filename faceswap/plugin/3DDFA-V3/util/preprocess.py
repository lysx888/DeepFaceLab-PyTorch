...
def back_resize_ldms(ldms, trans_params, target_size=224):
    
    w0, h0, s, t = trans_params[0], trans_params[1], trans_params[2], [trans_params[3],trans_params[4]]
...
def back_resize_crop_img(img, trans_params, ori_img, target_size=224, resample_method = Image.BICUBIC):
...
def resize_n_crop_img(img, lm, t, s, target_size=224., mask=None):
    ...
    w = int(w0*s)
    h = int(h0*s)
    tx = np.asarray(t[0]).item()
    ty = np.asarray(t[1]).item()
