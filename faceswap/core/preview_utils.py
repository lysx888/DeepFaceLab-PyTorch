...
def draw_loss_chart(
    ...
    abs_max = float(np.percentile(all_losses, 95)) * 1.5
    if abs_max <= 0:
        abs_max = 1.0

    for ci in range(num_curves):
        losses = np.array([h[1 + ci] for h in history], dtype=np.float64)
        color = loss_colors[ci]
        lh_len = len(losses)
        cols = np.arange(width)
        idx = (cols * (lh_len - 1) / max(width - 1, 1)).astype(int)
        vals = losses[idx]
        y = np.clip((vals / abs_max) * (height - 1), 0, height - 1).astype(int)
        rows = height - y - 1
        points = np.stack([cols, rows], axis=1).reshape(-1, 1, 2).astype(np.int32)
        cv2.polylines(chart, [points], isClosed=False, color=color, thickness=1)
...
def compose_preview(
    ...
    for i, row in enumerate(section_rows):
        if row.dtype != np.uint8:
            _logger.warning(f"section_row[{i}] dtype={row.dtype}, converting")
