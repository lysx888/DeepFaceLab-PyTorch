...
class MetadataManager:
    ...
    def save(image_path: Path, metadata: FaceMetadata) -> None:
        json_path = MetadataManager.sidecar_path(Path(image_path))
        json_str = orjson.dumps(metadata.to_dict())
    ...
    def pack_metadata(aligned_dir: Path, output_path: Path) -> None:
        ...
        json_str = orjson.dumps(pack_data)
    ...
    _INDEX_FILENAME = "_source_index.json"

    @staticmethod
    def build_source_index(aligned_dir: Path, progress_callback=None) -> dict[str, list[str]]:
        aligned_dir = Path(aligned_dir)
        index: dict[str, list[str]] = {}
        if not aligned_dir.exists():
            return index
        files = [f for f in aligned_dir.iterdir()
                 if f.is_file() and f.suffix.lower() in (".jpg", ".jpeg", ".png")]
        total = len(files)
        for i, f in enumerate(files):
            meta = MetadataManager.load(f, lightweight=True)
            if meta is not None and meta.source_filename:
                key = Path(meta.source_filename).stem
            else:
                key = f.stem.rsplit("_", 1)[0]
            index.setdefault(key, []).append(f.name)
            if progress_callback is not None:
                progress_callback(i + 1, total)
        index_path = aligned_dir / MetadataManager._INDEX_FILENAME
        try:
            json_str = orjson.dumps(index)
            FileManager.atomic_write(index_path, json_str)
        except Exception as e:
            _logger.warning(f"Failed to write source index: {e}")
        return index

    @staticmethod
    def load_source_index(aligned_dir: Path) -> Optional[dict[str, list[str]]]:
        aligned_dir = Path(aligned_dir)
        index_path = aligned_dir / MetadataManager._INDEX_FILENAME
        ...
        try:
            ...
            dir_mtime = aligned_dir.stat().st_mtime
    ...
    @staticmethod
    def get_source_index(aligned_dir: Path) -> dict[str, list[str]]:
        index = MetadataManager.load_source_index(aligned_dir)
        if index is not None:
            return index
        return MetadataManager.build_source_index(aligned_dir)

    @staticmethod
    def get_next_aligned_number(aligned_dir: Path) -> int:
        index = MetadataManager.get_source_index(aligned_dir)
        max_num = -1
        for aligned_names in index.values():
            for name in aligned_names:
                stem_part = Path(name).stem.rsplit("_", 1)[0]
                try:
                    num = int(stem_part)
                    if num > max_num:
                        max_num = num
    ...
    @staticmethod
    def add_to_source_index(aligned_dir: Path, source_stem: str, aligned_filename: str):
        aligned_dir = Path(aligned_dir)
        index = MetadataManager.get_source_index(aligned_dir)
        entries = index.setdefault(source_stem, [])
        if aligned_filename not in entries:
            entries.append(aligned_filename)
            index_path = aligned_dir / MetadataManager._INDEX_FILENAME
            try:
                json_str = orjson.dumps(index)
                FileManager.atomic_write(index_path, json_str)
            except Exception as e:
                _logger.warning(f"Failed to update source index: {e}")

    @staticmethod
    def remove_from_source_index(aligned_dir: Path, source_stem: str, aligned_filename: str):
        aligned_dir = Path(aligned_dir)
        index = MetadataManager.get_source_index(aligned_dir)
        entries = index.get(source_stem)
        if entries and aligned_filename in entries:
            entries.remove(aligned_filename)
            if not entries:
                del index[source_stem]
            index_path = aligned_dir / MetadataManager._INDEX_FILENAME
            try:
                json_str = orjson.dumps(index)
                FileManager.atomic_write(index_path, json_str)
