...
class FaceMetadata:
    ...
    def get_xseg_mask_array(self, h: int = 0, w: int = 0) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if h > 0 and w > 0:
                mask_uint8 = mask_uint8.reshape(h, w)
            return mask_uint8
...
class MetadataManager:
    ...
    def _sidecar_path(image_path: Path) -> Path:
        return MetadataManager.sidecar_path(image_path)
    ...
    def load_all(aligned_dir: Path, lightweight: bool = False) -> dict[str, FaceMetadata]:
        ...
        _logger.info(f"[DIAG] load_all: find_images found {len(_paths)} files in {time.time()-_t0:.2f}s")
        _t1 = time.time()
        ...
        with ThreadPoolExecutor(max_workers=min(8, max(2, len(_paths) // 100))) as pool:
            for item in pool.map(_load_one, _paths):
                if item is not None:
                    name, meta = item
                    result[name] = meta

        _logger.info(f"[DIAG] load_all: parsed {len(result)} metadata files in {time.time()-_t1:.2f}s")
        return result
