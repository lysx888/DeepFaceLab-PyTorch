...
def linear_color_transfer(target_img: np.ndarray, source_img: np.ndarray,
    ...
    matched_img += mu_s
    return np.clip(matched_img.astype(np.float32), [0, -127, -127], [100, 127, 127])
...
def color_transfer_lab_match(target: np.ndarray, source: np.ndarray,
    ...
    s_l_mean, s_l_std = _masked_mean_std(source_lab[..., 0], source_mask, mask_cutoff)
    l_ratio = 0.3
    result_lab[..., 0] = target_lab[..., 0] + l_ratio * (s_l_mean - t_l_mean)

    ab_t = target_lab[..., 1:].reshape(-1, 2).astype(np.float64)
    ab_s = source_lab[..., 1:].reshape(-1, 2).astype(np.float64)
    mu_t = ab_t.mean(axis=0)
    mu_s = ab_s.mean(axis=0)
    Ct = np.cov(ab_t.T) + 1e-6 * np.eye(2)
    Cs = np.cov(ab_s.T) + 1e-6 * np.eye(2)
    eva_t, eve_t = np.linalg.eigh(Ct)
    eva_s, eve_s = np.linalg.eigh(Cs)
    Ct_inv_half = eve_t @ np.diag(1.0 / np.sqrt(np.clip(eva_t, 1e-8, None))) @ eve_t.T
    Cs_half = eve_s @ np.diag(np.sqrt(np.clip(eva_s, 1e-8, None))) @ eve_s.T
    W = Cs_half @ Ct_inv_half
    ab_result = (ab_t - mu_t) @ W.T + mu_s
    result_lab[..., 1] = ab_result[:, 0].reshape(target_lab.shape[:2])
    result_lab[..., 2] = ab_result[:, 1].reshape(target_lab.shape[:2])

    np.clip(result_lab[..., 0], 0, 100, out=result_lab[..., 0])
...
def color_transfer_rct_partial(target: np.ndarray, source: np.ndarray,
    ...
    mean_diff = float(np.mean(np.abs(target_lab.mean(axis=(0, 1)) - source_lab.mean(axis=(0, 1)))))
    adaptive_blend = float(np.clip(0.3 + 1.5 * mean_diff / 100.0, 0.1, 0.9))
    transferred = reinhard_color_transfer(target, source,
                                          target_mask=target_mask,
                                          source_mask=source_mask,
                                          mask_cutoff=mask_cutoff)
    return (1.0 - adaptive_blend) * target + adaptive_blend * transferred
...
def color_transfer(ct_mode: str, img_src: np.ndarray, img_trg: np.ndarray,
                   ...
                   mask_cutoff: float = 0.5) -> np.ndarray:
    mk = dict(target_mask=src_mask, source_mask=trg_mask, mask_cutoff=mask_cutoff)
    if ct_mode == 'lct':
        src_lab = cv2.cvtColor(img_src.astype(np.float32), cv2.COLOR_BGR2LAB)
        trg_lab = cv2.cvtColor(img_trg.astype(np.float32), cv2.COLOR_BGR2LAB)
        out_lab = linear_color_transfer(src_lab, trg_lab)
        return np.clip(cv2.cvtColor(out_lab.astype(np.float32), cv2.COLOR_LAB2BGR), 0.0, 1.0)
    elif ct_mode == 'rct':
