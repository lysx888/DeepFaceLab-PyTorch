...
WFLW_MISSING_IF_106 = [38, 92, 80, 85, 84, 83, 82, 81]


def wflw_98_to_if_106(wflw_landmarks: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    lm106 = np.zeros((106, 2), dtype=np.float32)
    visible = np.ones(106, dtype=np.float32)
    for wflw_idx in range(98):
        if_idx = WFLW_TO_IF_106_MAP[wflw_idx]
        lm106[if_idx] = wflw_landmarks[wflw_idx]
    for if_idx in WFLW_MISSING_IF_106:
        visible[if_idx] = 0.0
    return lm106, visible
