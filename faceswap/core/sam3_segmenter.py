...
class SAM3Segmenter:
    ...
    def __init__(self):
        ...
        self._device = None
        # 持有上一张图的引用做身份比较(用 is 而非 id): id 会在对象被 GC 后复用地址,
        # 同分辨率新图分配到旧地址会误命中, 导致用上一张图的 embedding 分割当前图。
        self._current_image_ref: Optional[np.ndarray] = None
    ...
    def _set_image(self, image_bgr: np.ndarray):
        ...
        if self._current_image_ref is image_bgr and self._current_state is not None:
            return
        image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
        pil_image = PILImage.fromarray(image_rgb)
        with self._get_amp_ctx():
            self._current_state = self._processor.set_image(pil_image)
        self._current_image_ref = image_bgr
