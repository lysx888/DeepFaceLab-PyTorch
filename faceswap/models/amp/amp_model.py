...
class AMPModel(BaseModel):
    _model_prefix = "amp"
    _gui_config_filename = "AMP_training_config.json"
    _param_labels = {
    ...
    def build(self) -> None:
        ...
        self.inter_src.requires_grad_(False)
        self.inter_dst.requires_grad_(False)
    ...
    def build_optimizers(self) -> None:
        ...
        if lr_dropout_cfg in ('y', 'cpu') and not self.pretrain:
            lr_dropout = 0.3
            lr_cos = 500
        elif lr_dropout_cfg == 'ca' and not self.pretrain:
            lr_dropout = 1.0
            lr_cos = 0
            target_iter = getattr(c, 'target_iter', 0)
            lr_anneal = target_iter if target_iter > 0 else 10000
        else:
            lr_dropout = 1.0
            lr_cos = 0
        ...
        opt = AdaBelief(gen_params, lr=lr, lr_dropout=lr_dropout,
                        lr_cos=lr_cos, lr_anneal=lr_anneal, clipnorm=clipnorm)
