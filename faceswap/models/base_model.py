...
class BaseModel(ABC):
    ...
    def _preview_predict_src(self, warped: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        return self._predict_src(warped)

    def _preview_predict_dst(self, warped: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        return self._predict_dst(warped)

    def _append_preview(self, result: dict[str, list], S, SS, D, DD, SD, Sm, Dm, DDm, SDm) -> None:
        name = self._preview_section_name
        result[name].append([S, SS, D, DD, SD])
        result[f'{name} masked'].append([S * Sm, SS, D * Dm, DD * DDm, SD * SDm])
    ...
    def generate_preview_data(self, src_dataset, dst_dataset,
        ...
        with torch.no_grad():
            for i in range(n):
                ...
                s_warped = s_sample['warped_image'].unsqueeze(0).to(self.device)
                S = s_sample['target_image'].unsqueeze(0).to(self.device)
                Sm = s_sample['target_mask'].unsqueeze(0).to(self.device)
                d_warped = d_sample['warped_image'].unsqueeze(0).to(self.device)
                D = d_sample['target_image'].unsqueeze(0).to(self.device)
                Dm = d_sample['target_mask'].unsqueeze(0).to(self.device)

                SS, _ = self._preview_predict_src(s_warped)
                DD, DDm = self._preview_predict_dst(d_warped)
                SD, SDm = self._preview_predict_src(d_warped)

                S, SS, D, DD, SD = S[0], SS[0], D[0], DD[0], SD[0]
                Sm, Dm, DDm, SDm = Sm[0], Dm[0], DDm[0], SDm[0]
                self._append_preview(result, S, SS, D, DD, SD, Sm, Dm, DDm, SDm)
        self._set_train()
        return result
    ...
    def named_parameters(self):
        for mod_name, module in self._modules_dict.items():
    ...
    def training_mode(self):
        return any(m.training for m in self._modules_dict.values())

    def get_merge_face(self, warped_dst: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        self._set_eval()
        with torch.no_grad():
            pred, pred_mask = self._predict_src(warped_dst)
            _, dst_mask = self._predict_dst(warped_dst)
        self._set_train()
    ...
    def try_load(self) -> None:
        ...
        lr_anneal = getattr(self, '_lr_anneal', 0)
        prev_ca_active = self._aux_state.get('ca_active', False)
        newly_ca = lr_anneal > 0 and not prev_ca_active
        for opt in self._optimizers_dict.values():
            for group in opt.param_groups:
                group['lr_anneal'] = lr_anneal
                if newly_ca:
                    group['lr_anneal_start'] = iter_count
    ...
    def save(self, iter_count: int) -> None:
        ...
        try:
            ...
            if hasattr(self.config, 'to_dict'):
                ...
                # 同步写回 GUI 使用的配置文件（如 SAEHD_training_config.json），
                # 让下次打开界面能看到训练实际使用的参数（含 auto_batch 探测结果）。
                if self._gui_config_filename:
                    (tmp_dir / self._gui_config_filename).write_text(
                        json.dumps(config_dict, indent=2, ensure_ascii=False), encoding='utf-8')
            training_state = {'iter': iter_count}
            training_state['ca_active'] = getattr(self, '_lr_anneal', 0) > 0
    ...
    def _get_backup_files(self) -> list[Path]:
        ...
        for fname in [self._prefixed('training_state.json'),
                      ...
                      self._prefixed('rng_state.pth'),
                      self._prefixed('cuda_rng_state.pth'),
                      self._prefixed('dataset_index.json'),
                      self._prefixed('gan_state.json')]:
