...
class IFLandmarkLoss(nn.Module):
    ...
    def forward_per_sample(self, pred: torch.Tensor, target: torch.Tensor,
                           progress: float = 1.0,
                           visible: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        warmup_end = self.warmup_ratio
        blend_start = warmup_end - self.blend_ratio
        ...
        per_sample_denom = mask.sum(dim=1).clamp(min=1.0)
        return per_sample_loss, per_sample_denom
