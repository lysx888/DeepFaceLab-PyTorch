...
class XSegNet(nn.Module):
    def __init__(self, resolution: int = 256, base_ch: int = 32):
        ...
        self.dense1 = nn.Linear(4 * 4 * b * 8, 512)
        self.dense2 = nn.Linear(512, 4 * 4 * b * 8)
        ...
        self.out_conv_pretrain = nn.Conv2d(b, 3, 3, 1, 1)

    def forward(self, x: torch.Tensor, skip_enabled: bool = True, pretrain: bool = False) -> torch.Tensor:
        x = self.conv01(x)
        x = x0 = self.conv02(x)
        x = self.bp0(x)
        ...
        if pretrain:
            return self.out_conv_pretrain(x)
