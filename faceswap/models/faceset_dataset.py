...
class FacesetDataset(Dataset):
    def __init__(
        ...
        self._metadata_cache_max = 4096
        self._image_cache_max = min(512, max(32, 200 * 1024 * 1024 // (resolution * resolution * 3)))
        ...
        if pretrain_mode:
            ...
            for k, v in raw_meta.items():
                if v.seg_ie_polys is not None:
                    self._image_paths.append(self._aligned_dir / k)
            if not self._image_paths:
                raise ValueError(f"No annotated faces found in {self._aligned_dir}")

        self._metadata_cache_max = max(4096, len(self._image_paths))

    def __getstate__(self):
        state = self.__dict__.copy()
        # 必须保持 OrderedDict: _get_meta 依赖 move_to_end (fork 后 worker 里会调用)
    ...
    def merge(cls, datasets: list["FacesetDataset"]) -> "FacesetDataset":
        ...
        merged._image_paths = []
        merged._metadata_cache = collections.OrderedDict()
        merged._image_cache_max = min(512, max(32, 200 * 1024 * 1024 // (merged._resolution * merged._resolution * 3)))
        merged._image_cache = collections.OrderedDict()
        for ds in datasets:
            merged._image_paths.extend(ds._image_paths)
        merged._metadata_cache_max = max(4096, len(merged._image_paths))
        return merged
