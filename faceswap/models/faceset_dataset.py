...
from faceswap.shared.image_utils import bgr_to_rgb, bgr_to_hsv, hsv_to_bgr
...
class FacesetDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self._pretrain_mode:
            ...
            if self._augment and np.random.random() < 0.5:
                img = np.fliplr(img).copy()
            img_rgb = bgr_to_rgb(img)
            img_f = img_rgb.astype(np.float32) / 255.0
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            gray_rgb = np.stack([gray, gray, gray], axis=-1)
            gray_f = gray_rgb.astype(np.float32) / 255.0
            img_tensor = torch.from_numpy(img_f).permute(2, 0, 1)
            target_tensor = torch.from_numpy(gray_f).permute(2, 0, 1)
            return {
                "image": img_tensor,
    ...
    def _render_mask(self, shape: tuple, meta: FaceMetadata) -> np.ndarray:
        ...
        if meta.seg_ie_polys is None:
            _logger.warning(f"render_mask: seg_ie_polys is None for {getattr(meta, 'source_filename', '?')}, returning empty mask")
            return mask
