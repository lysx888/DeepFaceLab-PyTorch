...
class XSegModel(BaseModel):
    ...
    def build_optimizers(self) -> None:
        ...
        # 与 DFL Model_XSeg 一致：RMSprop(lr=1e-4, lr_dropout=0.3)，固定 mask 语义
        # （mask 存优化器 state、只乘 update，二阶矩 a 用未 mask 的 grad）
        from faceswap.models.saehd.optimizers import RMSprop
        optimizer = RMSprop(self.xseg_net.parameters(), lr=lr, rho=0.9,
                            lr_dropout=self.config.lr_dropout)
        self.register_optimizer('xseg_opt', optimizer)
        # DFL XSeg 无学习率调度（lr 恒定），故不设置 scheduler
