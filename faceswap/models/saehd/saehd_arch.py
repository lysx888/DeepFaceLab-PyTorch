...
padding=kernel_size//2 与 DFL 完全一致：DFL 的 Conv2D 将 'SAME' 重定义为对称 padding=((k-1)*dil+1)//2（见 core/leras/layers/Conv2D.py），
即卷积前对输入做对称 k//2 零填充，与 PyTorch Conv2d(padding=kernel_size//2) 一致（含 stride=2 下采样）。

注意：PyTorch 的 F.pixel_shuffle 与 TF 的 tf.nn.depth_to_space 通道排列不同，
转换脚本 dfl_models_to_pytorch.py 中会对权重做通道 permutation 以适配 pixel_shuffle。
"""
import torch
...
class Decoder(nn.Module):
    def __init__(self, in_ch, d_ch, d_mask_ch, opts=''):
        ...
        self.out_conv = nn.Conv2d(d_ch * 2, 3, kernel_size=3, padding=1)
        if use_d:
