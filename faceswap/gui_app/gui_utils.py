...
import cv2
import numpy as np
from PyQt6.QtCore import QPointF, QRectF, QObject, QEvent
from PyQt6.QtWidgets import QSpinBox, QDoubleSpinBox, QComboBox, QToolTip

from faceswap.setting import FaceType, DATA_SRC_ALIGNED_DIR, DATA_DST_ALIGNED_DIR
from faceswap.core.metadata_manager import FaceMetadata, MetadataManager
...
class _PersistentToolTipFilter(QObject):
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.ToolTip:
            tip = obj.toolTip()
            if tip:
                QToolTip.showText(event.globalPos(), tip, obj, msecShowTime=300000)
                return True
        return False


_persistent_tooltip_filter = _PersistentToolTipFilter()


def install_persistent_tooltip(widget):
    widget.installEventFilter(_persistent_tooltip_filter)
    return widget
    ...
    if not skip_aligned:
        ...
        if existing_face_path is not None:
            ...
            if existing_meta is not None:
                ...
                if existing_meta.xseg_mask is not None:
                    meta.xseg_mask = existing_meta.xseg_mask
                if existing_meta.pose is not None:
                    meta.pose = existing_meta.pose
        else:
            from faceswap.business.face_extractor import _compute_pose
            meta.pose = _compute_pose(kps_5, kps_5_visibility=kps_5_visibility)
            ext = f".{output_format.lower().lstrip('.')}"
            next_num = MetadataManager.get_next_aligned_number(aligned_dir)
            face_path = aligned_dir / f"{next_num:05d}_0{ext}"
        ...
        if existing_face_path is None:
            MetadataManager.add_to_source_index(aligned_dir, Path(source_filename).stem, face_path.name)
    else:
        face_path = None

    debug_dir = (aligned_dir.parent / (aligned_dir.name + "_debug")) if not skip_aligned else None
    bbox_tuple = None
    if source_rect is not None and len(source_rect) >= 4:
        bbox_tuple = (source_rect[0], source_rect[1], source_rect[2], source_rect[3])
    debug_stem = Path(source_filename).stem
    if debug_dir is not None:
        try:
            save_debug_image(debug_dir, debug_stem, source_img, lm_106, transform_mat, output_size, bbox=bbox_tuple)
...
def _save_insightface_training_data(meta: FaceMetadata, source_image_path: Optional[Path], source_filename: str, face_idx: int = 0):
    try:
        from faceswap.business.insightface_training_data_generator import InsightFaceTrainingDataGenerator
        from faceswap.setting import WORKSPACE_DIR
