...
GROUP_COLORS = {
    ...
    ParamGroup.TRAINING_CONTROL: "#B8860B",
}
...
GROUP_NAMES = {
    ...
    ParamGroup.TRAINING_CONTROL: "训练控制",
}
...
class ConfigManager:
    ...
    def save_config(self, values: dict) -> None:
        try:
            ...
            FileManager.atomic_write(self._config_path, json.dumps(values, indent=2, ensure_ascii=False))
    ...
    def load_config(self) -> Optional[dict]:
        ...
        try:
            ...
            _logger.warning(f"Failed to load config from {self._config_path}: {e}")
            return None
...
class ParamGroupWidget(QGroupBox):
    ...
    def _build_rows(self) -> None:
        ...
        def _flush():
            nonlocal cur_row, cur_type, cur_max
            if cur_row:
                rows.append(cur_row)
            cur_row = []
            cur_type = None
        ...
        for p in self._params:
            ...
            if p.render_hint == "new_row":
                _flush()
            elif p.render_hint == "row_4":
                _flush()
                cur_max = 4
            elif cur_type is not None and cur_type != p_type:
                _flush()
            elif len(cur_row) >= cur_max:
                _flush()
