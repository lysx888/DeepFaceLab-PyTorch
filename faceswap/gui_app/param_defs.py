...
class ArchiButtonGroup(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._df_radio = QRadioButton("DF")
        self._liae_radio = QRadioButton("LIAE")
        self._df_radio.setChecked(True)
        self._df_radio.toggled.connect(self._on_changed)
        self._liae_radio.toggled.connect(self._on_changed)

        btn_style = (
            "QRadioButton { font-weight: bold; font-size: 13px; "
            "padding: 4px 12px; border: 1px solid #888; border-radius: 4px; }"
            "QRadioButton::checked { background: #0078D4; color: white; border-color: #0078D4; }"
        )
        self._df_radio.setStyleSheet(btn_style)
        self._liae_radio.setStyleSheet(btn_style)
        layout.addWidget(self._df_radio)
        layout.addWidget(self._liae_radio)

        sep = QLabel("|")
        sep.setStyleSheet("color: #999; font-size: 14px;")
        layout.addWidget(sep)

        self._mod_checks: dict[str, QCheckBox] = {}
        for key, tooltip in self._MODIFIERS:
            cb = QCheckBox(key)
    ...
    def value(self) -> str:
        arch = "df" if self._df_radio.isChecked() else "liae"
        suffix = "".join(k for k, cb in self._mod_checks.items() if cb.isChecked())
        return f"{arch}-{suffix}" if suffix else arch

    def set_value(self, val: str) -> None:
        val = val.strip()
        if val.startswith("liae"):
            self._liae_radio.setChecked(True)
            suffix = val[4:]
        else:
            self._df_radio.setChecked(True)
            suffix = val[2:]
        if suffix.startswith("-"):
            suffix = suffix[1:]
        for k, cb in self._mod_checks.items():
            cb.setChecked(k in suffix)
...
class ParamGroupWidget(QGroupBox):
    ...
    def _build_rows(self) -> None:
        ...
        for p in self._params:
            full_row = p.render_hint == "archi_buttons"

            if full_row or row_layout is None or col_count >= 3:
                if row_layout is not None:
                    remaining = 3 - col_count
            ...
            row_layout.addLayout(unit, 3 if full_row else 1)
    ...
    def _create_widget(self, param: ParamDef) -> QWidget:
        if param.render_hint == "archi_buttons":
            w = ArchiButtonGroup()
            w.set_value(str(param.default))
            return w
    ...
    def get_values(self) -> dict:
        ...
        for key, widget in self._param_widgets.items():
            ...
            if isinstance(widget, ArchiButtonGroup):
                result[key] = widget.value()
            elif pdef.type == ParamType.INT:
                result[key] = widget.value()
