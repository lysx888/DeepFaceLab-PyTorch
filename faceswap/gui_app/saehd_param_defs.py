...
SAEHD_PARAM_DEFS: list[ParamDef] = [
    ParamDef(
        ...
        tooltip="训练分辨率(像素)。必须是16的倍数(含'd'或't'架构需32的倍数)。\n"
    ...
    ParamDef(
        ...
        tooltip="模型架构:\n"
                ...
                "  t = 深层(更多downscale+resblock, 需32倍数)\n"
                "  u = 像素归一化(pixel_norm)",
    ),
    ParamDef(
    ...
    ParamDef(
        ...
        tooltip="学习率策略(对SRC和DST对称生效):\n"
                "n  = 不启用(推荐初期)\n"
                "y  = DFL模式: 30%参数随机冻结 + 余弦波震荡(lr_cos=500)\n"
                "ca = 余弦退火: 关闭冻结和震荡, 学习率单调下降到1%\n"
                "     T_max=target_iter(>0时)否则10000步\n"
                "     建议target_iter设为本次训练期望总步数,\n"
                "     从开启ca的时刻起满lr退火到lr/100",
    ),
    ParamDef(
    ...
    ParamDef(
        ...
        group=ParamGroup.BASIC,
        tooltip="使用预训练数据集(大量不同人脸)进行预训练。\n"
                "数据源固定为 workspace/pretrain_faces/faceset.pak (名称锁定)\n"
                "训练时自动解压到该目录, 无需手动操作\n"
                "该 pak 不存在时, 自动使用 src/dst 数据兜底预训练\n"
    ...
    ParamDef(
        key="auto_batch", label="自动批次探测", type=ParamType.BOOL, default=False,
        group=ParamGroup.OPTIMIZATION,
        tooltip="启用后自动探测最大不OOM的batch size。\n"
                "探测结果不会超过手动设置的批次大小。\n"
                "每次训练开始时探测一次，约需10-30秒。",
    ),
    ParamDef(
        key="gradient_accumulation_steps", label="梯度累积步数:", type=ParamType.INT, default=1,
        min_val=1, max_val=16, group=ParamGroup.OPTIMIZATION,
        tooltip="梯度累积步数，等效增大batch size。\n"
                "1=不累积(默认)\n"
                "n=每n步才更新一次参数，等效batch_size×n\n"
                "适用于显存不够但想用大batch训练的场景",
    ),
]
...
SAEHD_SUBGROUPS: list = [
    ...
    SubGroupDef(
        ...
        param_keys=["adabelief", "lr", "lr_dropout", "clipgrad"],
    ...
    SubGroupDef(
        ...
        param_keys=["amp_mode", "gradient_accumulation_steps", "enable_torch_compile", "auto_batch"],
