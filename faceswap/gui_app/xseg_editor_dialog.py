...
from PyQt6.QtWidgets import (
    ...
    QSplitter, QSizePolicy, QApplication, QFrame, QMessageBox,
)
...
class XSegEditorDialog(QDialog):
    ...
    def _refresh_annotated_count(self):
        if not hasattr(self, '_meta_cache') or self._meta_cache is None:
            self._meta_cache = MetadataManager.load_all(Path(self._images[0]).parent) if self._images else {}
        count = 0
        for p in self._images:
            m = self._meta_cache.get(Path(p).name)
            if m is not None and m.seg_ie_polys is not None:
                count += 1
    ...
    def _auto_mask_sam3(self):
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).error(f"SAM3自动分割失败: {e}", exc_info=True)
        finally:
            QApplication.restoreOverrideCursor()
            from faceswap.core.sam3_segmenter import SAM3Segmenter
