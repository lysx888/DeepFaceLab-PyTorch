...
from faceswap.gui_app.annotator_common import _resize_pad, load_thumbnail
...
class _XSegCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._baked_qimage: Optional[QImage] = None
    ...
    def _save_current_polys(self):
        ...
        if self._from_xseg_mask:
            ...
            meta.xseg_mask = None
        if self._polys:
            meta.seg_ie_polys = current_dump
        else:
            meta.seg_ie_polys = None
        MetadataManager.save(self._img_path, meta)
        self._polys_original_dump = current_dump
    ...
    def _bake_mask(self):
        ...
        if mask is not None:
            ...
            self._baked_qimage = QImage(rgb.data, w, h, 3 * w, QImage.Format.Format_RGB888).copy()
        else:
            self._baked_qimage = None
    ...
    def paintEvent(self, event):
        ...
        if self._op_mode == _OpMode.VIEW_BAKED and self._baked_qimage is not None:
            base_image = self._baked_qimage
...
class _ThumbLoadWorker(QThread):
    thumb_ready = pyqtSignal(str, object)

    def __init__(self, paths: list[Path], indices: list[int]):
        super().__init__()
        self._paths = paths
    ...
    def run(self):
        for idx in self._indices:
            ...
            qimg = load_thumbnail(self._paths[idx], _CENTER_THUMB_W, _CENTER_THUMB_H)
            if qimg is not None:
                self.thumb_ready.emit(name, QPixmap.fromImage(qimg))
...
class _BottomThumbBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._current_idx = -1
        self._worker: Optional[_ThumbLoadWorker] = None
        self.setFixedHeight(80)
        self.setMinimumWidth(400)
        self.setMouseTracking(True)

    def _on_thumb_ready(self, name: str, pm: QPixmap):
    ...
    def set_current(self, idx: int):
        ...
        to_load = []
        for slot in range(_VISIBLE_COUNT):
            i = idx - _CENTER_POS + slot
            if 0 <= i < len(self._paths) and self._pixmaps.get(self._paths[i].name) is None:
                to_load.append(i)
        if to_load:
            self._worker = _ThumbLoadWorker(self._paths, to_load)
            self._worker.thumb_ready.connect(self._on_thumb_ready)
            self._worker.start()
        self._trim_cache()
        self.update()
...
class _ImageLoaderThread(QThread):
    ...
    def run(self):
        ...
        if self._stop:
            return
        self.finished_loading.emit(images, -1)

        import orjson as _orjson

        json_paths = list(self._aligned_dir.glob("*.json"))
        total = len(json_paths)

        def _check_annotated(json_path):
            try:
                with open(str(json_path), "rb") as f:
                    data = _orjson.loads(f.read())
                seg = data.get("seg_ie_polys")
                return seg is not None and len(seg) > 0
            except Exception:
                return False
        ...
        for i, jp in enumerate(json_paths):
            if self._stop:
                break
            if _check_annotated(jp):
                annotated_count += 1
            if i % 50 == 0 or i == total - 1:
                self.progress.emit(i + 1, total, jp.name)
        if not self._stop:
            self.progress.emit(total, total, "")
            self.count_ready.emit(annotated_count)


class XSegEditorDialog(QDialog):
    _sig_auto_mask_done = pyqtSignal(str, object)

    def __init__(self, aligned_dir: Path, parent=None):
        super().__init__(parent)
        self._sig_auto_mask_done.connect(self._on_auto_mask_done)
        ...
        self._current_was_annotated = False
        ...
        self._loading_label = QLabel("正在扫描目录...")
        self._loading_label.setStyleSheet("color: #CCCCCC; font-size: 16px;")
        self._loading_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        loading_layout.addWidget(self._loading_label)
        self._loading_progress = QProgressBar()
        self._loading_progress.setStyleSheet("QProgressBar { color: #CCCCCC; }")
        self._loading_progress.setRange(0, 1)
        ...
        self._loader_thread.count_ready.connect(self._on_count_ready)
        self._loader_thread.progress.connect(self._on_load_progress)
        self._loader_thread.start()

    def done(self, result):
        self._thumb_bar._stop_worker()
    ...
    def _on_load_progress(self, current, total, name):
        if hasattr(self, '_loading_progress') and self._loading_progress is not None:
            self._loading_progress.setRange(0, total)
            self._loading_progress.setValue(current)
        if name and hasattr(self, '_loading_label') and self._loading_label is not None:
            self._loading_label.setText(f"正在加载元数据... {current}/{total}")
    ...
    def _on_load_finished(self, images, annotated_count):
        ...
        self._annotated_label.setText("已标记: 统计中...")
    ...
    def _refresh_annotated_count(self):
        now_annotated = self._canvas.get_current_has_polys()
        if now_annotated and not self._current_was_annotated:
            self._annotated_count += 1
        elif not now_annotated and self._current_was_annotated:
            self._annotated_count -= 1
        self._current_was_annotated = now_annotated
        self._annotated_label.setText(f"已标记: {self._annotated_count}")
    ...
    def _load_image_at(self, idx: int):
        ...
        self._current_was_annotated = self._canvas.get_current_has_polys()
        self._annotated_label.setText(f"已标记: {self._annotated_count}")
    ...
    def _auto_mask_face_parsing(self):
        ...
        image = self._canvas._image.copy() if self._canvas._image is not None else None

        def _task():
            try:
                from faceswap.core.face_masker import FaceMasker
                masker = FaceMasker.get_instance()
                result = masker.auto_draw_mask(image, use_occlusion=True)
                self._sig_auto_mask_done.emit("face_parsing", result)
            except Exception as e:
                _logger.error(f"Face Parsing自动遮罩失败: {e}", exc_info=True)
                self._sig_auto_mask_done.emit("face_parsing", None)
    ...
    def _auto_mask_dfl(self):
        ...
        def _task():
            try:
                from faceswap.core.face_masker import FaceMasker
                masker = FaceMasker.get_instance()
                result = masker.auto_draw_mask_dfl(image)
                self._sig_auto_mask_done.emit("dfl", result)
            except Exception as e:
                _logger.error(f"DFL遮罩自动绘制失败: {e}", exc_info=True)
                self._sig_auto_mask_done.emit("dfl", None)
    ...
    def _auto_mask_sam3(self):
        ...
        def _task():
            try:
                from faceswap.core.sam3_segmenter import SAM3Segmenter
                segmenter = SAM3Segmenter.get_instance()
                include_polys, exclude_polys = segmenter.segment_auto(image)
                self._sig_auto_mask_done.emit("sam3", (include_polys, exclude_polys))
            except Exception as e:
                _logger.error(f"SAM3自动分割失败: {e}", exc_info=True)
                self._sig_auto_mask_done.emit("sam3", None)

        threading.Thread(target=_task, daemon=True).start()

    def _on_auto_mask_done(self, method: str, data):
        ...
        if data is None:
            return
        if method in ("face_parsing", "dfl"):
            self._canvas._apply_auto_mask_result(data)
        elif method == "sam3":
            include_polys, exclude_polys = data
            for poly_pts in include_polys:
                poly = _SegIEPoly(_PolyType.INCLUDE)
                for x, y in poly_pts:
                    poly.pts.append(QPointF(x, y))
                self._canvas._polys.append(poly)
            for poly_pts in exclude_polys:
                poly = _SegIEPoly(_PolyType.EXCLUDE)
                for x, y in poly_pts:
                    poly.pts.append(QPointF(x, y))
                self._canvas._polys.append(poly)
            if self._canvas._polys:
                self._canvas._cur_poly_idx = len(self._canvas._polys) - 1
                self._canvas._op_mode = _OpMode.EDIT_PTS
            self._canvas._notify_mode_changed()
            self._canvas.update()
