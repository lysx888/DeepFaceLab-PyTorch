...
from PyQt6.QtCore import pyqtSignal, QObject, Qt, QThread, QSize
...
class _DedupThumbLoader(QThread):
    thumb_loaded = pyqtSignal(int, QPixmap)

    def __init__(self, paths: list[Path], thumb_size: int):
        super().__init__()
        self._paths = paths
        self._thumb_size = thumb_size
        self._stop = False
    ...
    def run(self):
        for i, p in enumerate(self._paths):
            if self._stop:
                break
            qimg = load_thumbnail(p, self._thumb_size, self._thumb_size, pad=False)
            if qimg is not None:
                self.thumb_loaded.emit(i, QPixmap.fromImage(qimg))
...
class ThumbnailDialog(QDialog):
    def __init__(self, title, images, aligned_dir=None, parent=None):
        ...
        self._thumb_loader = _DedupThumbLoader(images, self._thumb_size)
    ...
    def closeEvent(self, event):
        if hasattr(self, '_thumb_loader') and self._thumb_loader is not None:
...
class SrcDedupDialog(QDialog):
    def __init__(self, aligned_dir, parent=None):
        ...
        def _param_row(items):
            row = QHBoxLayout()
            for i, (label, spin) in enumerate(items):
                if i > 0:
                    row.addSpacing(16)
                lbl = QLabel(label)
        ...
        self._occlusion_thresh = QDoubleSpinBox()
        self._occlusion_thresh.setRange(0.05, 0.95)
        self._occlusion_thresh.setSingleStep(0.05)
        self._occlusion_thresh.setValue(0.45)
        install_no_wheel(self._occlusion_thresh)

        self._protect_roll = QDoubleSpinBox()
        self._protect_roll.setRange(0.05, 0.95)
        self._protect_roll.setSingleStep(0.05)
        self._protect_roll.setValue(0.25)
        self._protect_roll.setToolTip("原图归一化歪头 |roll|>阈值 直接保护不删 (1.0≈90°)")
        install_no_wheel(self._protect_roll)

        _param_row([("每簇保留:", self._keep_per_cluster), ("大角度保护:", self._protect_yaw)])
        _param_row([("角度网格:", self._yaw_grid), ("表情阈值:", self._exp_thresh)])
        _param_row([("光照阈值:", self._sh_thresh), ("批大小:", self._batch_size)])
        _param_row([("遮挡阈值:", self._occlusion_thresh), ("歪头保护:", self._protect_roll)])
    ...
    def _on_run(self):
        ...
        params = dict(
            ...
            occlusion_thresh=self._occlusion_thresh.value(),
            protect_roll=self._protect_roll.value(),
            use_cpu=self._use_cpu.isChecked(),
        )
