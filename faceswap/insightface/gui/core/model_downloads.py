...
def is_model_package_installed(model_name: str, model_root: str | os.PathLike[str]) -> bool:
    target = Path(model_root).expanduser() / model_name
...
def download_model_asset(
    ...
    model_root_path = Path(model_root).expanduser()
