...
from ..setting import INSIGHTFACE_ROOT
...
class FaceAnalysis:
    def __init__(self, name=DEFAULT_MP_NAME, root=None, allowed_modules=None, **kwargs):
        if root is None:
        ...
        for onnx_file in onnx_files:
            ...
            if model is None:
                ...
                import logging; logging.getLogger(__name__).debug('find model: %s %s %s %s %s', onnx_file, model.taskname, model.input_shape, model.input_mean, model.input_std)
    ...
    def prepare(self, ctx_id, det_thresh=0.5, det_size=None):
        ...
        import logging; logging.getLogger(__name__).debug('set det-size: %s', det_size)
    ...
    def get_batch(self, imgs, max_num=0, det_metric='default', batch_size=8):
        results = []
        for start in range(0, len(imgs), batch_size):
            batch_imgs = imgs[start:start + batch_size]
            batch_det_results = self.det_model.detect_batch(
                batch_imgs, max_num=max_num, metric=det_metric
            )
            for i, img in enumerate(batch_imgs):
                bboxes, kpss = batch_det_results[i]
                if bboxes.shape[0] == 0:
                    results.append([])
                    continue
                faces = []
