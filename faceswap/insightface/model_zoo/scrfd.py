...
class SCRFD:
    ...
    def detect_batch(self, imgs, input_size=None, max_num=0, metric='default'):
        input_sizes = self._resolve_input_sizes(input_size)
        assert input_sizes
        all_dets = []
        all_kpss = []
        for ins in input_sizes:
            batch_dets, batch_kpss = self._detect_batch_candidates(imgs, ins)
            all_dets.append(batch_dets)
            if self.use_kps:
                all_kpss.append(batch_kpss)
        results = []
        for i in range(len(imgs)):
            pre_det_list = []
            kpss_det_list = []
            for ins_idx, ins in enumerate(input_sizes):
                pre_det = all_dets[ins_idx][i]
                kpss = all_kpss[ins_idx][i] if self.use_kps and ins_idx < len(all_kpss) else None
            ...
            if not pre_det_list:
                results.append((np.empty((0, 5), dtype=np.float32), None))
                continue
            pre_det = np.vstack(pre_det_list).astype(np.float32, copy=False)
            order = pre_det[:, 4].argsort()[::-1]
            pre_det = pre_det[order, :]
            kpss = np.vstack(kpss_det_list)[order, :, :] if kpss_det_list and len(kpss_det_list) == len(pre_det_list) else None
            keep = self.nms(pre_det)
            det = pre_det[keep, :]
            kpss = kpss[keep, :, :] if kpss is not None else None
            if max_num > 0 and det.shape[0] > max_num:
                area = (det[:, 2] - det[:, 0]) * (det[:, 3] - det[:, 1])
                img_center = imgs[i].shape[0] // 2, imgs[i].shape[1] // 2
                offsets = np.vstack([
    ...
    def _detect_batch_candidates(self, imgs, input_size):
        N = len(imgs)
        blobs = []
        det_scales = []
        for img in imgs:
        ...
        per_img_dets = [[] for _ in range(N)]
        per_img_kpss = [[] for _ in range(N)]
        for idx, stride in enumerate(self._feat_stride_fpn):
            if self.batched:
                scores_all = net_outs[idx]
                bbox_preds_all = net_outs[idx + self.fmc]
                kps_preds_all = net_outs[idx + self.fmc * 2] if self.use_kps else None
            else:
                scores_all = net_outs[idx]
                bbox_preds_all = net_outs[idx + self.fmc]
                kps_preds_all = net_outs[idx + self.fmc * 2] if self.use_kps else None
            for bi in range(N):
                if self.batched:
                    scores = scores_all[bi]
                    bbox_preds = bbox_preds_all[bi] * stride
                    kps_preds = kps_preds_all[bi] * stride if kps_preds_all is not None else None
                else:
                    s_start = bi * (scores_all.shape[1] if scores_all.ndim > 1 else scores_all.shape[0] // N)
                    s_end = s_start + (scores_all.shape[1] if scores_all.ndim > 1 else scores_all.shape[0] // N)
                    scores = scores_all[bi]
                    bbox_preds = bbox_preds_all[bi] * stride
                    kps_preds = kps_preds_all[bi] * stride if kps_preds_all is not None else None
                ...
                pos_scores = scores[pos_inds]
                pos_bboxes = bboxes[pos_inds]
                per_img_dets[bi].append(pos_scores)
                per_img_dets[bi].append(pos_bboxes)
                if self.use_kps and kps_preds is not None:
                    kpss = distance2kps(anchor_centers, kps_preds)
                    kpss = kpss.reshape((kpss.shape[0], -1, 2))
                    pos_kpss = kpss[pos_inds]
                    per_img_kpss[bi].append(pos_kpss)
        results_det = []
        results_kpss = []
        for bi in range(N):
            scores_parts = per_img_dets[bi][0::2]
            bboxes_parts = per_img_dets[bi][1::2]
            kps_parts = per_img_kpss[bi]
            if not scores_parts or sum(s.size for s in scores_parts) == 0:
                results_det.append(np.empty((0, 5), dtype=np.float32))
                kps_shape = (0, 5, 2) if self.use_kps else None
                results_kpss.append(np.empty(kps_shape, dtype=np.float32) if kps_shape else None)
                continue
            scores = np.vstack(scores_parts)
            bboxes = np.vstack(bboxes_parts) / det_scales[bi]
            order = scores.ravel().argsort()[::-1]
            pre_det = np.hstack((bboxes, scores)).astype(np.float32, copy=False)
            pre_det = pre_det[order, :]
            kpss = np.vstack(kps_parts) / det_scales[bi] if kps_parts else None
            kpss = kpss[order, :, :] if kpss is not None else None
            results_det.append(pre_det)
            results_kpss.append(kpss)
        return results_det, results_kpss
...
def get_scrfd(name, download=False, root=None, **kwargs):
    if root is None:
        from ..setting import MODEL_ROOT
        root = MODEL_ROOT
    if not download:
