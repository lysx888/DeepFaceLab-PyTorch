...
_SWAPPED_EXT_PRIORITY = {".jpg": 0, ".jpeg": 1, ".png": 2, ".bmp": 3, ".webp": 4}
...
class ModelMerger:
    ...
    def composite_to_frames(
        ...
        if swapped_dir.exists():
            for f in swapped_dir.iterdir():
                if f.is_file():
                    existing = swapped_map.get(f.stem)
                    if existing is None or _SWAPPED_EXT_PRIORITY.get(f.suffix.lower(), 99) < _SWAPPED_EXT_PRIORITY.get(existing.suffix.lower(), 99):
                        swapped_map[f.stem] = f
        ...
        for i, frame_path in enumerate(dst_frames):
            ...
            for face_name, meta in faces_for_frame:
                swapped_path = swapped_map.get(Path(face_name).stem)
