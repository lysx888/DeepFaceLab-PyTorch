...
class XSegTrainer:
    ...
    def train(
        ...
        if use_lr_dropout:
            _logger.info(f"lr_dropout={lr_dropout}: dynamic per-step binary mask (对齐DFL)")
                ...
                if use_lr_dropout:
                    for p in model.parameters():
                        if p.grad is not None:
                            p.grad.mul_(torch.bernoulli(torch.full_like(p, lr_dropout)))
    ...
    def apply_auto_mask(
        ...
        _logger.info(f"Applied auto mask ({method}) to {count} faces in {aligned_dir}")
        return count
