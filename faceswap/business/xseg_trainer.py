...
class XSegTrainer:
    ...
    def cleanup(self):
        try:
            if hasattr(self, '_xseg_model') and self._xseg_model is not None:
                if hasattr(self._xseg_model, 'xseg_net') and self._xseg_model.xseg_net is not None:
                    self._xseg_model.xseg_net.cpu()
                self._xseg_model = None
        except Exception:
            pass
    ...
    def train(
        ...
        if src_dir.exists():
            try:
                src_ds = FacesetDataset(src_dir, resolution=resolution, augment=True, face_type=face_type)
            except ValueError:
                src_ds = None
        else:
            src_ds = None
        if dst_dir.exists():
            try:
                dst_ds = FacesetDataset(dst_dir, resolution=resolution, augment=True, face_type=face_type)
            except ValueError:
                dst_ds = None
        else:
            dst_ds = None
        ...
        if pretrain:
            ...
            if pretrain_dir.exists():
                try:
                    pretrain_ds = FacesetDataset(pretrain_dir, resolution=resolution, augment=True, pretrain_mode=True, face_type=face_type)
                except ValueError:
                    pass
            if pretrain_ds is None and pretrain:
                ...
                if src_dir.exists():
                    try:
                        pretrain_ds = FacesetDataset(src_dir, resolution=resolution, augment=True, pretrain_mode=True, face_type=face_type)
                    except ValueError:
                        pass
        ...
        use_lr_dropout = 0 < lr_dropout < 1.0
        ...
        target_str = f"target {target_iter} iters" if target_iter > 0 else "no target (infinite)"
        _logger.info(f"XSeg training started: {n_bce} annotated images, {n_pt} pretrain images, {target_str}, device={device}")
        if pretrain:
        ...
        if 'pretrain_iter' in saved_config:
            pretrain_iter = saved_config['pretrain_iter']
        if pretrain and pretrain_iter == 0:
            pretrain_iter = 10000
        ...
        try:
            ...
            while not self._stop_event.is_set() and (target_iter == 0 or iter_count < target_iter):
                ...
                if cur_pretrain and pt_iter is not None:
                    ...
                    with amp.autocast():
                        pred = model(imgs, skip_enabled=False)
                    ...
                    with amp.autocast():
                        pred = model(imgs, skip_enabled=True)
                    pred = pred.float()
                ...
                if torch.isnan(loss).any() or torch.isinf(loss).any():
                    _logger.warning(f"Skipping step: loss={loss.item():.6f} (NaN/Inf)")
                    amp.update()
                    iter_count += 1
                    self._iter_count = iter_count
                    self._loss_history.append((iter_count, 0.0))
                    if len(self._loss_history) > 10000:
                        del self._loss_history[:5000]
                    if on_iter is not None and not self._stop_event.is_set():
                        on_iter(iter_count, 0.0, 0.0)
                    continue
                ...
                for p in model.parameters():
                    if p.grad is not None and (p.grad.isnan().any() or p.grad.isinf().any()):
                        ...
                        break
                if grad_has_nan:
                    _logger.warning(f"Skipping step: NaN/Inf in gradients at iter {iter_count}")
                    amp.update()
                    iter_count += 1
                    self._iter_count = iter_count
                    self._loss_history.append((iter_count, 0.0))
    ...
    def _sample_train_images(self, n: int) -> list[tuple[np.ndarray, np.ndarray]]:
        ...
        for p in sampled:
            ...
            meta = self._preview_train_ds._get_meta(p) if self._preview_train_ds else None
    ...
    def _preview_row_pretrain(
        ...
        with torch.inference_mode():
            pred = torch.sigmoid(model(img_t, skip_enabled=False).squeeze().cpu()).numpy()
        pred = np.clip(pred, 0.0, 1.0)

        col1 = I
        col2 = np.stack([pred, pred, pred], axis=-1)

        row = np.concatenate([col1, col2], axis=1)
        row = np.clip(row, 0, 1)
        return (row * 255).astype(np.uint8)
    ...
    def _preview_row_train(
        ...
        img_t = torch.from_numpy(I).permute(2, 0, 1).unsqueeze(0).to(device)
        with torch.inference_mode():
            logits = model(img_t, skip_enabled=True).squeeze().cpu()
    ...
    def apply_trained_mask(
        ...
        for idx, img_path in enumerate(all_paths):
            ...
            img_t = torch.from_numpy(resized.astype(np.float32) / 255.0).permute(2, 0, 1).unsqueeze(0).to(device)
    ...
    def apply_auto_mask(
        ...
        aligned_dir = Path(aligned_dir)
        all_paths = list(FileManager.find_images(aligned_dir))
        total = len(all_paths)
        t0 = time.time()
        count = 0
        ...
        for idx, img_path in enumerate(all_paths):
            ...
            if method == "face_parsing":
                ...
                result = masker.auto_draw_mask(img, use_occlusion=True)
                include_polys = result.include_polys
                exclude_polys = result.exclude_polys
            elif method == "dfl":
                from faceswap.core.face_masker import FaceMasker
                masker = FaceMasker.get_instance()
                result = masker.auto_draw_mask_dfl(img)
                include_polys = result.include_polys
                exclude_polys = result.exclude_polys
            elif method == "sam3":
                from faceswap.core.sam3_segmenter import SAM3Segmenter
                segmenter = SAM3Segmenter.get_instance()
                include_polys, exclude_polys = segmenter.segment_auto(img)
            else:
                continue

            if not include_polys and not exclude_polys:
                continue
            ...
            if meta is None:
                continue
            meta.xseg_mask = FaceMetadata.encode_xseg_mask(mask)
            MetadataManager.save(img_path, meta)
            count += 1
