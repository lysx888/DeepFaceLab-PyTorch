...
from faceswap.setting import FFMPEG_PATH, FFPROBE_PATH
...
class VideoProcessor:
    def __init__(self, ffmpeg_path: Optional[str] = None) -> None:
        ...
        self._ffprobe = self._ffmpeg.parent / f"ffprobe{'.exe' if __import__('sys').platform == 'win32' else ''}"
        ...
        if not self._ffprobe.exists():
            alt_probe = Path("ffprobe")
            try:
                subprocess.run([str(alt_probe), "-version"], capture_output=True, check=True)
                self._ffprobe = alt_probe
            except (FileNotFoundError, subprocess.CalledProcessError):
                raise FileNotFoundError(
                    f"ffprobe not found at {self._ffprobe}. "
                    f"Please install ffprobe and set the correct path."
                )

    def _run_ffmpeg(self, args: list[str], silent: bool = False,
                     stream_callback=None, stop_check=None) -> subprocess.CompletedProcess:
    ...
    def extract_frames_src(
        ...
        args = []
        if limit_threads:
            args += ["-threads", str(min((os.cpu_count() or 4) // 2, 4))]
    ...
    def denoise_frames(self, frames_dir: Path,
                        h: float = 10, h_color: float = 10,
                        template_window: int = 7, search_window: int = 21) -> None:
    ...
    def merge_frames_to_video(
        ...
        ext = images[0].suffix.lstrip('.') if images else 'png'
        if ext not in ('jpg', 'jpeg', 'png', 'bmp'):
            ext = 'png'

        args = [
            "-framerate", str(fps),
            "-i", str(frames_dir / f"%05d.{ext}"),
        ]
        ...
        if lossless:
            args += ["-c:v", "libx264", "-crf", "0", "-pix_fmt", "yuv420p"]
        else:
        ...
        self._run_ffmpeg(args, stream_callback=stream_callback, stop_check=stop_check)
        return output_path
