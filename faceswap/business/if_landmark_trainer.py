...
from faceswap.shared.hardware_utils import probe_batch_size, get_available_ram_gb, image_cache_size, assert_no_oom
...
class IFLandmarkTrainer:
    ...
    def train(
        ...
        extra_data_dirs: Optional[list[Path]] = None,
        accum_steps: int = 1,
        ...
        _cache_n = image_cache_size(get_available_ram_gb())
        _logger.info(f"Image cache size: {_cache_n} per worker")
        ...
        if extra_data_dirs:
            ...
            for d in extra_data_dirs:
                if Path(d).exists():
                    ...
                    _logger.info(f"Loaded extra dataset: {d} ({len(extra_ds)} samples)")
            if extra_datasets:
                dataset = IFLandmarkDataset.merge([dataset] + extra_datasets)
                _logger.info(f"Concat dataset total: {len(dataset)} samples")
        ...
        if auto_batch and is_gpu:
            probed_bs = probe_batch_size(if_model.if_net, _INPUT_SIZE, device)
            batch_size = min(probed_bs, batch_size)
            _logger.info(f"Auto batch size: {batch_size}")
        ...
        num_datasets = dataset.num_datasets
        sample_ds_ids = dataset.sample_dataset_ids
        use_dynamic_sampling = num_datasets > 1

        if use_dynamic_sampling:
            from torch.utils.data import WeightedRandomSampler
            ds_weights = [1.0] * num_datasets
            sample_weights = [ds_weights[sample_ds_ids[i]] for i in range(len(dataset))]
            ...
            _logger.info(f"Dynamic weighted sampling enabled for {num_datasets} datasets")
        else:
            loader = DataLoader(
                dataset,
                batch_size=effective_bs,
        ...
        ds_loss_accum = [0.0] * num_datasets
        ds_count_accum = [0] * num_datasets
        ds_weight_smooth = [1.0] * num_datasets
        ...
        for epoch in range(start_epoch, target_epoch):
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                _per_sample_vals = None

                def _fwd_loss_bwd():
                    nonlocal _per_sample_vals
                    p = net(images)
                    if use_dynamic_sampling:
                        ps_loss, ps_denom = loss_fn.forward_per_sample(p, labels, progress=progress, visible=visibles)
                        l = ps_loss.sum() / ps_denom.sum().clamp(min=1.0)
                        _per_sample_vals = (ps_loss.detach() / ps_denom.detach()).cpu().tolist()
                    else:
                        l = loss_fn(p, labels, progress=progress, visible=visibles)
                    (l / accum_steps).backward()
                    return l
                loss = assert_no_oom(_fwd_loss_bwd)
                ...
                if use_dynamic_sampling:
                    ds_ids_batch = batch['dataset_id'].tolist()
                    for i, ds_id in enumerate(ds_ids_batch):
                        ds_loss_accum[ds_id] += _per_sample_vals[i]
                        ds_count_accum[ds_id] += 1
            ...
            if use_dynamic_sampling and any(c > 0 for c in ds_count_accum):
                epsilon = 1e-4
                for ds_id in range(num_datasets):
                    if ds_count_accum[ds_id] > 0:
                        ds_avg_loss = ds_loss_accum[ds_id] / ds_count_accum[ds_id]
                        new_w = ds_avg_loss + epsilon
                        ds_weight_smooth[ds_id] = 0.7 * ds_weight_smooth[ds_id] + 0.3 * new_w
                total_w = sum(ds_weight_smooth)
                ds_weights = [w * num_datasets / total_w for w in ds_weight_smooth]
                sample_weights = [ds_weights[sample_ds_ids[i]] for i in range(len(dataset))]
                sampler.weights = torch.tensor(sample_weights, dtype=torch.double)
                ds_loss_accum = [0.0] * num_datasets
                ds_count_accum = [0] * num_datasets
                _logger.info(f"Dynamic sampling weights updated: {[f'{w:.3f}' for w in ds_weights]}")
