...
from faceswap.setting import FaceType, DEFAULT_FACE_OUTPUT_SIZE, DEFAULT_DET_THRESH, DEFAULT_JPG_QUALITY, FACE_TYPE_SCALE, FACE_TYPE_CHIN_OFFSET, LANDMARK_POINTS, KPS5_POINTS
...
def _compute_yaw(kps_5: np.ndarray) -> Optional[float]:
    ...
    if not left_ok:
        ...
        if face_width < 1.0:
            return None
        return float((nose[0] - right_eye[0]) / face_width)
    if not right_ok:
        face_width = abs(nose[0] - left_eye[0]) * 2.0
        if face_width < 1.0:
            return None
        return float((nose[0] - left_eye[0]) / face_width)
