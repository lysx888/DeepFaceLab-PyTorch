...
class FaceSorter:
    def sort_aligned(
        ...
        def _report(done, total):
            if progress_callback is not None:
                ...
                speed = f"{done / elapsed:.1f}it/s" if elapsed > 0 else ""
                progress_callback(done, total, elapsed, remaining, speed)
        ...
        return self._rename_sorted(aligned_dir, scored, algorithm, index_progress_callback)

    def _rename_sorted(self, aligned_dir: Path, scored: list[tuple[float, Path]], algorithm: SortAlgorithm, index_progress_callback=None) -> int:
    ...
    def _oneface(self, aligned_dir: Path, images: list[Path], metadata_map: dict[str, FaceMetadata]) -> int:
        ...
        for img_path in images:
            meta = metadata_map.get(img_path.name)
            source = meta.source_filename if meta else img_path.stem.rsplit("_", 1)[0]
    ...
    def _compute_yaw(img_path: Path, meta: Optional[FaceMetadata]) -> float:
        if meta is None or meta.landmarks_106 is None or len(meta.landmarks_106) < 106:
            return 0.0
        lm = meta.landmarks_106.astype(np.float64)
        left_eye = lm[[93, 96, 94, 95, 89, 90, 87, 91]].mean(axis=0)
        right_eye = lm[[35, 41, 40, 42, 39, 37, 33, 36]].mean(axis=0)
        nose = lm[86]
