...
def compute_sharpness(img_path: Path) -> float:
    """整脸清晰度：中心 80% 区域 Tenengrad 梯度平均能量 (mean 已按像素归一化).

    不用眼部 ROI：侧脸/大角度脸眼睛缺失时不会被误判为模糊；
    取中心 80% 避开对齐边缘填充和纯色背景对梯度的稀释。
    """
...
def load_landmarks_106(img_path: Path) -> Optional[np.ndarray]:
    """读取头像 json 元数据中的 aligned 坐标 106 点 (凸包掩膜用)."""
...
def compute_occlusion_ratio(img_path: Path) -> float:
    """大面积遮挡粗判：人脸掩膜内低梯度(平坦)像素占比, 返回 0~1.

    掩膜优先取 landmarks_106 凸包, 缺失时退化为中心椭圆。
    遮挡(手/物体盖脸)表现为大片平坦区域, 占比越高越可疑。
    """
    img = cv2.imread(str(img_path), cv2.IMREAD_GRAYSCALE)
    if img is None:
        return 1.0
    ...
    lm = load_landmarks_106(img_path)
    if lm is not None:
        hull = cv2.convexHull(lm.astype(np.float32)).astype(np.int32)
        cv2.fillConvexPoly(mask, hull, 255)
    else:
        cv2.ellipse(mask, (w // 2, int(h * 0.45)),
                    (int(w * 0.38), int(h * 0.5)), 0, 0, 360, 255, -1)
    if (mask > 0).sum() == 0:
        return 1.0
    gx = cv2.Sobel(img, cv2.CV_64F, 1, 0)
    gy = cv2.Sobel(img, cv2.CV_64F, 0, 1)
    mag = cv2.magnitude(gx, gy)
    flat = np.logical_and(mag < 8.0, mask > 0).sum()
    return float(flat / (mask > 0).sum())


def load_roll_from_metadata(img_path: Path) -> Optional[float]:
    """读取元数据 json 中的归一化 roll (pose[2], 范围[-1,1], 1.0≈90°).

    仅用于歪头保护：原图 roll 明显(如 |roll|>=阈值)的图直接保护不删，
    因为对齐会把 roll 摆正，aligned 图跑 3DDFA 看不到原图歪头差异。
    """
    json_path = img_path.with_suffix(".json")
    if not json_path.exists():
        return None
...
def load_pose_degrees_from_metadata(img_path: Path) -> Optional[tuple]:
    """读取元数据 pose 并转回角度(度): [pitch, yaw, roll].

    元数据 pose 为归一化 [-1,1] (1.0≈90°), 来自 InsightFace 检测或 5 点估算,
    是原图空间姿态 —— 尤其 roll 保留了真实歪头 (对齐后 3DDFA 测的 roll≈0)。
    缺失时返回 None, 由调用方回退 3DDFA 角度。
    """
    json_path = img_path.with_suffix(".json")
    if not json_path.exists():
        return None
...
def dedup_states(
    ...
    occlusion_thresh: float = 0.45,
    protect_roll: float = 0.25,
):
    bins = defaultdict(list)
    protected = []

    for s in states:
        if s.get("occlusion", 0.0) >= occlusion_thresh:
            protected.append(s)
            continue
        if abs(s.get("roll_norm", 0.0)) >= protect_roll:
            protected.append(s)
            continue
...
def generate_report(
    ...
    occluded = [s for s in keep if s.get("occlusion", 0.0) >= occlusion_thresh]
    lines.append(f"遮挡保护:    {len(occluded)} (平坦像素占比>={occlusion_thresh})")
    rolled = sum(1 for s in keep if abs(s.get("roll_norm", 0.0)) >= protect_roll)
    lines.append(f"歪头保护:    {rolled} (|roll|>={protect_roll}, 原图归一化)")
    lines.append("")

    if occluded:
        lines.append("-" * 70)
        lines.append("遮挡保护名单 (不会被删除)")
        lines.append("-" * 70)
        for s in sorted(occluded, key=lambda x: -x.get("occlusion", 0.0))[:50]:
            lines.append(f"  {s['path'].name}  平坦占比={s.get('occlusion', 0.0):.2f}  yaw={s['yaw']:.1f}")
        lines.append("")
...
def analyze_states(
    ...
    """
    分析头像状态, 返回 [(path, yaw, pitch, roll, exp, sh), ...].
    exp: 64维表情向量 (3DDFA), sh: 27维光照SH系数 (3DDFA)。
    yaw/pitch/roll: 优先读元数据 pose (原图空间, 尤其 roll 保留真实歪头), 缺失时回退 3DDFA 角度。
    ...
    for i in range(0, total, batch_size):
        ...
        for j, alpha in enumerate(alphas):
            ...
            pose_deg = load_pose_degrees_from_metadata(valid_paths[j])
...
def run_dedup(
    ...
    for i in range(0, total, batch_size):
        ...
        for j, alpha in enumerate(alphas):
            ...
            occlusion = compute_occlusion_ratio(valid_paths[j])
            all_states.append({
                "path": valid_paths[j],
                "yaw": yaw,
                "pitch": pitch,
...
def main():
    ...
    parser.add_argument("--occlusion_thresh", type=float, default=0.45, help="遮挡保护阈值(平坦像素占比0~1, 达到即保护不删)")
    parser.add_argument("--protect_roll", type=float, default=0.25, help="歪头保护阈值(原图归一化roll, 1.0≈90°, |roll|达到即保护不删)")
    ...
    keep, remove, report_text = run_dedup(
        ...
        protect_roll=args.protect_roll,
