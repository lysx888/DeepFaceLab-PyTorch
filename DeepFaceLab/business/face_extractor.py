...
from DeepFaceLab.setting import FaceType, DEFAULT_FACE_OUTPUT_SIZE, DEFAULT_DET_THRESH, DEFAULT_JPG_QUALITY, FACE_TYPE_SCALE, FACE_TYPE_CHIN_OFFSET, LANDMARK_POINTS, KPS5_POINTS
from DeepFaceLab.core.insightface_adapter import InsightFaceAdapter, DetectedFace, AlignedFace
from DeepFaceLab.core.metadata_manager import MetadataManager, FaceMetadata
from DeepFaceLab.core.debug_image_generator import draw_debug_image
...
def _compute_yaw(kps_5: np.ndarray) -> Optional[float]:
    ...
    if not left_ok and not right_ok:
        return None
    if not left_ok:
        dist = abs(nose[0] - right_eye[0])
        if dist < 1.0:
            return None
        return float(dist / max(abs(right_eye[0]), 1.0) * 0.5 + 0.8)
    if not right_ok:
        dist = abs(nose[0] - left_eye[0])
        if dist < 1.0:
            return None
        return -(float(dist / max(abs(left_eye[0]), 1.0) * 0.5 + 0.8))
    eye_mid = (left_eye + right_eye) / 2
    eye_dist = abs(left_eye[0] - right_eye[0])
    if eye_dist < 1.0:
        return None
    return float((nose[0] - eye_mid[0]) / eye_dist)
        ...
        try:
            ...
            for f_idx, face in enumerate(faces):
                aligned = adapter.align_face(
                    img, face.landmarks_106, face_type, output_size, kps_5=face.kps_5
                )
                aligned_faces.append((face, aligned))

            debug_info = None
            if debug_output:
                debug_img = draw_debug_image(img, face.landmarks_106, aligned.transform_matrix, output_size, face_rect=_bbox_to_face_rect(face.bbox))
            ...
            for f_idx, (face, aligned) in enumerate(aligned_faces):
                ...
                meta = FaceMetadata(
                    ...
                    arcface_embedding=face.embedding.astype(np.float32) if face.embedding is not None else None,
                    yaw=_compute_yaw(face.kps_5),
        ...
        for i, img_path in enumerate(images):
            ...
            for idx, face in enumerate(faces):
                aligned = self._adapter.align_face(
                    img, face.landmarks_106, config.face_type, config.output_size, kps_5=face.kps_5
                )
                aligned_faces.append((face, aligned))
