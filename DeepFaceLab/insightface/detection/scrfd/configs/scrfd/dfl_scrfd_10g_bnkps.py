...
_optimizer = dict(type='SGD', lr=0.01, momentum=0.9, weight_decay=0.0005)
...
_dfl_workspace = os.environ.get('DFL_WORKSPACE', os.path.join(os.getcwd(), 'workspace'))
data_root = os.path.join(_dfl_workspace, 'insightface_train', 'scrfd')
...
data = dict(
    ...
    train=dict(
        ...
        ann_file=os.path.join(train_root, 'labelv2.txt'),
        img_prefix=os.path.join(train_root, 'images'),
        pipeline=[
            dict(type='LoadImageFromFile', to_float32=True),
            dict(type='LoadAnnotations', with_bbox=True, with_keypoints=True),
            dict(
    ...
    val=dict(
        type='RetinaFaceDataset',
        ann_file=os.path.join(val_root, 'labelv2.txt'),
        img_prefix=os.path.join(val_root, 'images'),
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
