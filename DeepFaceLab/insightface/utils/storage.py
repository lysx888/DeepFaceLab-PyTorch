...
def download(sub_dir, name, force=False, root=None):
    if root is None:
...
def ensure_available(sub_dir, name, root=None):
    if root is None:
        root = INSIGHTFACE_ROOT
    return download(sub_dir, name, force=False, root=root)

def download_onnx(sub_dir, model_file, force=False, root=None, download_zip=False):
    if root is None:
        root = INSIGHTFACE_ROOT
    _root = os.path.expanduser(root)
