...
class InsightFaceAdapter:
    ...
    def _ensure_swap(self) -> None:
        ...
        try:
            import onnxruntime as ort
            swap_path = self._model_dir / INSIGHTFACE_SWAP_MODEL
            if not swap_path.exists():
                raise FileNotFoundError(f"INSwapper model not found: {swap_path}")
            providers = ["CUDAExecutionProvider", "CPUExecutionProvider"] if self._ctx_id >= 0 else ["CPUExecutionProvider"]
            self._swap_session = ort.InferenceSession(str(swap_path), providers=providers)
            _logger.info("INSwapper model loaded.")
        except Exception as e:
            raise RuntimeError(f"Failed to load INSwapper model: {e}") from e
    ...
    def align_face(
        ...
        if kps_5 is None:
            kps_5 = np.stack([
                (landmarks_106[34] + landmarks_106[38]) / 2,
                (landmarks_106[88] + landmarks_106[92]) / 2,
                landmarks_106[86],
                landmarks_106[52],
                landmarks_106[61],
            ]).astype(np.float32)
    ...
    def swap_face(
        ...
        if self._swap_model is None:
            self._swap_model = INSwapper(self._swap_session)
        result = self._swap_model.get(target_img, source_face, target_face, paste_back=paste_back)
        return result
